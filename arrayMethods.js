...
// CODE HERE
const evenNumbers = mixedNumbers.filter(numbers1 => numbers1 % 2 === 0)
...
// CODE HERE
const postTaxPrices = prices.map(priceNums => priceNums * 1.07);
...
// CODE HERE
const totalPopulation  = populations.reduce((startingValue, currentValue) => {
...
// CODE HERE
 const myStrongest  = monstersInYourPocket.filter(bigMonsters => bigMonsters.CP >= 200)
...
const orderAfterTaxTotal = orders.map(function(order) {
  return order.price + (order.price * order.tax)
})

console.log(orderAfterTaxTotal)
...
const bobsTotal = purchases.filter(bobPrice => bobPrice.owner === "Bob")
.reduce((totalPrice, startingPrice) => totalPrice + startingPrice.price, 0)
