...
const multiply = (num1, num2, callBack1) => {
  callBack1(num1 * num2)
}
...
const last = (anArray2, calback3) => {
  calback3(anArray2[anArray2.length - 1])
}
...
const contains = (theArray, aName, callback4) => {
  if (theArray.filter(aName => aName)){
    callback4(true)
  }else {
    callback4(false)
  }
...
// const each = (namesArray, callbackNames) => {
//   forEach(callbackNames)
// }
...
const getUserById = (objArray, ids, user) => {
  ids.id === ids
  user(objArray)``
}
